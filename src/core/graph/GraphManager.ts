...
export default class GraphManager extends GraphDrawing implements GraphManagerInterface {
    ...
    private readonly _backgroundLayer: PIXI.Container;
    ...
    private _buildBackground(bg: Background) {
        ...
        let background = PIXI.Sprite.from(bg.url);
        ...
        // });
        let hasMouseUp = true;
        background.on('pointerdown', (event: PIXI.InteractionEvent) => {
            hasMouseUp = true;
            setTimeout(() => {
                hasMouseUp = false
            }, 500);
        })
        background.on('pointerup', (event: PIXI.InteractionEvent) => {
            if (hasMouseUp) {
                this._app.stateManager.select(SelectEnum.None, []);
            }
        })
    ...
    private _changeAllShapesColor(isWhite: boolean, shapeIndex?: number) {
        this._shapeLayer.children.forEach((item: ShapeGraphics) => {
            ...
            let isVisible: boolean = shapeIndex != item.shapeIndex;
    ...
    setShapeContent(index: number, content?: ShapeContent): void {
        ...
        //店铺匹配时保存content
        this._graphCache.shapesContent[index] = content as ShapeContent;
